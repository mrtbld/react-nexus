...
function createPrepareSync(getBindings, fluxKey) {
  return function prepareSync(props, context) {
    const flux = context[fluxKey];
    const bindings = getBindings(props, flux);

    const promises = _.map(bindings, (binding) => {
      const state = flux.readStoreFromState(binding);
      if(state.isResolved()) {
        // During prepare, avoid re-fetching a store that's already been sucessfully fetched.
        return void 0;
      }
      const [store, query] = flux.matchStore(binding);
      const fetchResult = store.fetch(query);
      if(!_.isFunction(_.get(fetchResult, 'then'))) {
        return void 0;
      }
      return fetchResult;
    });
    return _.compact(promises);
  }
}


function createPrepareAsync(getBindings, fluxKey) {
  return async function prepareAsync(props, context) {
    const flux = context[fluxKey];
    const bindings = getBindings(props, flux);
...
function stores(
   ...
   */
  return (Component) => preparable(createPrepareAsync(getBindings, fluxKey))(class StoresComponent extends React.Component {
